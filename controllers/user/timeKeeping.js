...
exports.postCheckIn = (req, res, next) => {
  ...
  if (global.isCheckout || global.isCheckout == undefined) {
    ...
    res.render("user/checkIn", {
      mess: `You are check in at: ${new Date().toLocaleTimeString()}, workplace: ${
        req.body.workplace
      }`,
      name: req.user.name,
      isWorking: true,
    ...
    TimeKeeping.findOne({
      ...
      .then((timeKeepingInday) => {
        ...
        res.render("user/checkOut", {
          mess: `You Check Out Successful at: ${new Date().toLocaleTimeString()}`,
          name: req.user.name,
          path: "/checkOut",
          isAuth: req.session.isLoggedIn
        });
